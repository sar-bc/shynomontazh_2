...
$(document).ready(function () {
    ...
    $("#form").submit(function (e) {
        ...
        if (confirm(name + ', подтвердите вашу запись\n' + time_btn_press + ' ' + date)) {
            // Save it!
            console.log('Saved.');
            $.post("/api.php", form_data, function (data2) {
                console.log(data2);
    ...
    if (calendar) {
        ...
        if (btn_ref) {
            ...
            //запрашиваем с сервера по дате время
        $.get("/api.php?date=" + `${dat.value}`, function (get_data) {
            console.log(get_data);
            let div = document.getElementById("client");
                    if(div){
                    let tbody = document.getElementById("tab_client").getElementsByTagName("tbody")[0];
...
function handleDateChange(event) {
    ...
    $.get("/api.php?date=" + `${selectedDate}`, function (data) {
        //console.log(data);
        let div = document.getElementById("client");
                    if(div){
                    let tbody = document.getElementById("tab_client").getElementsByTagName("tbody")[0];
...
function del_client(id){
    ...
    if (confirm("Удалить запись " + td.textContent + "?")){
        ...
        $.get("/api.php?date=" + `${date.value}` + `&del=${id}`, function (data) {
            //console.log(data);
            let div = document.getElementById("client");
                        if(div){
                        let tbody = document.getElementById("tab_client").getElementsByTagName("tbody")[0];
